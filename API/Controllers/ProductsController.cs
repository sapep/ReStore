...
namespace API.Controllers
{
  public class ProductsController : BaseApiController
  {
    ...
    {
      List<string> brands = await _context.Products.Select(product => product.Brand).Distinct().ToListAsync();
      List<string> types = await _context.Products.Select(product => product.Type).Distinct().ToListAsync();
    ...
    public async Task<ActionResult<Product>> UpdateProduct([FromForm]UpdateProductDTO productDto)
    {
      ...
      _mapper.Map(productDto, product); // EF tracks the product
